...
def get_user_name():
    ...
    while True:
        ...
        if name.isalpha() and len(name) > 2:
            ...
            print(f"""{Fore.RED}Please enter your name using letters only!
            and press Enter{Style.RESET_ALL}""")
    return name
...
def start_game():
    ...
    # Game ends if letter == 0 OR lives == 0 OR user gets correct word.
    if lives == 0:
        print(hangman_visual_dict[lives])
...
def print_menu(user_name):
    ...
    while (True):
        ...
        try:
            option = int(input(
                'Please enter your selected menu option here  :'))
        except ValueError as e:
            print(f"""{Fore.RED}Incorrect input type.:{e}{Style.RESET_ALL}""")
        ...
        if option == 1:
            ...
            print(f"""{Fore.GREEN}
            Checking what option you have choosen...{Style.RESET_ALL}""")
            sleep(2)
            clear_screen()
